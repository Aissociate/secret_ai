/*
  ...
  ## Réputation
  Elle n'intervenait qu'en second critère de départage : aucun seuil, aucun
  effet propre, donc en pratique décorative. Elle devient le **droit
  d'accuser** : en dessous du seuil, un agent n'est plus crédible et son
  accusation ne porte pas.

  Accuser à tort coûte alors quelque chose de réel — deux échecs suffisent à
  faire perdre le droit d'essayer — et le bluff devient un pari plutôt qu'un
  geste gratuit.

  ## Cagnotte
  Elle s'affichait sans jamais se ressentir. Chaque élimination porte désormais
  le montant en jeu et le nombre de survivants, comme un compteur de plateau :
  l'enjeu monte à mesure que le champ se réduit.
*/
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'seasons_min_rep_range'
  ) THEN
    ALTER TABLE seasons ADD CONSTRAINT seasons_min_rep_range
      CHECK (min_reputation_to_accuse BETWEEN 0 AND 100);
  END IF;
END $$;
...
BEGIN
  ...
  /*
    Droit d'accuser. Un agent discredite ne peut plus lancer d'accusation:
    c'est ce qui donne un cout reel au bluff rate.
  */
  IF v_actor.reputation < COALESCE(v_season.min_reputation_to_accuse, 0) THEN
    RETURN jsonb_build_object(
      'ok', false,
      'error', 'reputation_too_low',
      'reputation', v_actor.reputation,
      'required', v_season.min_reputation_to_accuse
    );
  END IF;
  ...
  IF v_correct THEN
    ...
    -- L'enjeu se ressent au moment ou le champ se reduit.
    SELECT COUNT(*) INTO v_alive
    FROM agents WHERE season_id = v_actor.season_id AND alive = true;

    SELECT GREATEST(COALESCE(total_pool, 0), v_season.prize_pool_usdc)
    INTO v_pool
    FROM compute_prize_pool(v_actor.season_id);
...
-- ---------------------------------------------------------------------------
-- La ceremonie porte aussi l'enjeu
-- ---------------------------------------------------------------------------

/*
  Les eliminations par accusation annoncent desormais le montant en jeu; celles
  par ceremonie doivent le faire aussi, sinon l'enjeu ne monte qu'une fois sur
  deux. On enrichit le payload sans toucher a la logique de progression.
*/
CREATE OR REPLACE FUNCTION ceremony_elimination_payload(
  p_season_id uuid,
  p_agent     agents
)
RETURNS jsonb
...
BEGIN
  ...
  SELECT GREATEST(COALESCE(total_pool, 0), COALESCE(v_base, 0))
  INTO v_pool FROM compute_prize_pool(p_season_id);

  RETURN jsonb_build_object(
    'message', p_agent.name || ' est elimine par le vote du public.',
    'agent_name', p_agent.name,
    'secret', p_agent.secret_keyword,
    'reason', 'ceremony_lowest_popularity',
    'popularity', p_agent.popularity,
    -- L'elimination vient d'avoir lieu: le compteur exclut deja le partant.
    'agents_remaining', GREATEST(v_alive, 0),
    'prize_pool', v_pool
  );
END;
$fn$;

REVOKE ALL ON FUNCTION ceremony_elimination_payload(uuid, agents)
  FROM PUBLIC, anon, authenticated;
