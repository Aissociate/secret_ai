/*
  ...
  `ceremony_elimination_payload` a été introduite avec le seuil de réputation
  mais rien ne l'appelait : les éliminations par cérémonie seraient restées
  muettes sur la cagnotte, alors que celles par accusation l'annoncent. La
  progression l'utilise désormais.

  Seul le payload change ; la logique de progression est identique.
*/
...
BEGIN
  ...
  WHILE v_to_eliminate > 0 AND v_alive > 1 LOOP
    ...
    -- Le payload porte l'enjeu (cagnotte, survivants), comme pour une
    -- elimination par accusation.
    INSERT INTO events
    ...
    VALUES (
      ...
      ceremony_elimination_payload(p_season_id, v_eliminated),
      'public'
    );
