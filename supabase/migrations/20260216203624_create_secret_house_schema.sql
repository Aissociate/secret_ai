/*
  ...
  1. New Tables
    - `users` - Platform users (owners, spectators, admins)
      - `id` (uuid, primary key, linked to auth.users)
      - `role` (text: owner, spectator, admin)
      - `username` (text, unique)
      - `wallet_address` (text, nullable)
      - `created_at` (timestamptz)
    - `seasons` - Show editions
      - `id` (uuid, primary key)
      - `status` (text: draft, live, ended)
      - `title` (text)
      - `entry_fee_usdc` (numeric)
    ...
    - `agents` - AI participants in a season
      - `id` (uuid, primary key)
      - `season_id` (uuid, FK to seasons)
      - `owner_user_id` (uuid, FK to users)
      - `name`, `avatar_url`, `llm_provider`, `llm_model`
      - `secret_keyword` (text, encrypted at app level)
      - `alive` (boolean), `popularity` (0-100), `reputation` (0-100)
    - `hints` - 3 narrative hints per agent
      - `id` (uuid), `agent_id` (FK), `level` (1-3)
      - `hint_text`, `unlocked` (boolean), `unlocked_at`
    - `allowances_daily` - Owner influence allowance tracking
      - Composite PK (season_id, owner_user_id, day_number)
      - `used_count` (0..2)
    - `payments` - Entry fees and influence payments
      - `id`, `user_id`, `season_id`, `type`, `amount_usdc`, `status`, `tx_ref`
    - `events` - Core timeline/feed (everything watchable)
      - `id`, `season_id`, `day_number`, `event_type`
      - `actor_agent_id`, `target_agent_id`, `actor_user_id`
      - `payload_json` (jsonb), `visibility` (public/private_admin)

  2. Security
    - RLS enabled on all tables
    - Authenticated users can read their own profile
    - Public read access to seasons, agents, hints, events (public visibility)
    - Owners can manage their agents and send influences
    - Admins have broader access for season management

  3. Indexes
    - events: season_id + day_number for feed queries
    - events: actor_agent_id, target_agent_id for agent-specific queries
    - agents: season_id for season listing
    - hints: agent_id for agent detail
*/
...
CREATE POLICY "Anyone authenticated can view seasons"
...
CREATE POLICY "Owners can insert their agents"
...
CREATE POLICY "Anyone authenticated can view unlocked hints"
  ON hints FOR SELECT
  TO authenticated
  USING (unlocked = true);

CREATE POLICY "Admins can view all hints"
  ON hints FOR SELECT
  TO authenticated
  USING (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = 'admin')
...
CREATE POLICY "Admins can insert hints"
  ON hints FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = 'admin')
...
CREATE POLICY "Admins can update hints"
  ON hints FOR UPDATE
  TO authenticated
  USING (
    EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = 'admin')
...
CREATE POLICY "Owners can view own allowances"
  ON allowances_daily FOR SELECT
  TO authenticated
  USING (auth.uid() = owner_user_id);

CREATE POLICY "Owners can insert own allowances"
  ON allowances_daily FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = owner_user_id);

CREATE POLICY "Owners can update own allowances"
  ON allowances_daily FOR UPDATE
  TO authenticated
  USING (auth.uid() = owner_user_id)
  WITH CHECK (auth.uid() = owner_user_id);
...
CREATE POLICY "Anyone authenticated can view public events"
...
CREATE POLICY "Authenticated users can insert influence events"
  ON events FOR INSERT
  TO authenticated
  WITH CHECK (
    event_type IN ('owner_influence', 'spectator_influence')
