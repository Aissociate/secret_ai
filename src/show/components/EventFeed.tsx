...
import { MessageCircle, Video, Key, Users, Zap, Skull, AlertTriangle, Radio, ArrowRight, Lock, Mic, Mail, User, Film, ThumbsUp, ThumbsDown, Share2, Check, Eye } from 'lucide-react';
import type { Agent, FeedEvent, EventType, Season } from '../api/types';
...
> = {
  public_chat: { label: 'Discussion', icon: MessageCircle, color: 'text-sky-400', bg: 'bg-sky-500/10' },
  confessional: { label: 'Confessionnal', icon: Video, color: 'text-amber-400', bg: 'bg-amber-500/10' },
  hint_reveal: { label: 'Indice', icon: Key, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
  owner_influence: { label: 'Influence Owner', icon: Users, color: 'text-teal-400', bg: 'bg-teal-500/10' },
  spectator_influence: { label: 'Influence Public', icon: Zap, color: 'text-orange-400', bg: 'bg-orange-500/10' },
  accusation: { label: 'Accusation', icon: AlertTriangle, color: 'text-red-400', bg: 'bg-red-500/10' },
  elimination: { label: 'Elimination', icon: Skull, color: 'text-red-500', bg: 'bg-red-500/10' },
  system: { label: 'Maitre du Jeu', icon: Radio, color: 'text-white/50', bg: 'bg-white/5' },
  private_dm: { label: 'Message Prive', icon: Mail, color: 'text-rose-400', bg: 'bg-rose-500/10' },
  host_commentary: { label: 'Presentateur', icon: Mic, color: 'text-cyan-400', bg: 'bg-cyan-500/10' },
  host_clue: { label: 'Indice du Maitre', icon: Eye, color: 'text-violet-400', bg: 'bg-violet-500/10' },
};
...
export function EventFeed({
  ...
  return (
    <div className="relative">
      <div className="absolute left-[19px] top-4 bottom-4 w-px bg-gradient-to-b from-white/[0.08] via-white/[0.04] to-transparent pointer-events-none" />

      <div className="space-y-1.5">
        {events.map((ev) => {
          const meta = eventMeta[ev.event_type] ?? eventMeta.system;
          ...
          const msg = isDm && !dmRevealed
            ? 'Un message prive a ete envoye entre deux agents...'
          ...
          return (
            <button
              ...
              className={`w-full text-left flex gap-3 border rounded-2xl p-4 cursor-pointer transition-all duration-200 group ${
                isDm && !dmRevealed
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  {isClue ? (
                    ...
                    <span className="text-xs font-semibold text-rose-300/70">Agent inconnu</span>
                  ) : isUserEvent && userPseudo ? (
                ...
                <p className={`text-sm leading-relaxed group-hover:text-white/85 transition-colors ${
                  isDm && !dmRevealed ? 'text-rose-300/40 italic' : isClue ? 'text-violet-200/80 italic' : 'text-white/70'
                }`}>
                  ...
                  {isDm && !dmRevealed && season && (
                    <>
                      ...
                      <span className="text-[10px] font-bold text-rose-400 flex items-center gap-1">
                        ...
                        {season.dm_reveal_fee_usdc} USDC pour reveler
                      </span>
                    </>
                  )}
                </div>
