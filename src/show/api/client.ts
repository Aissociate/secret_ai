...
export async function fetchFeed(
  ...
  if (isDemoSeason(seasonId)) {
    ...
    // repetant « Jour 3 » puis « Jour 1 » puis « Jour 3 ».
    let events = [...DEMO_EVENTS].sort(
...
const CAREER_COLUMNS = 'config_id, owner_user_id, name, avatar_url, doctrine, rating, seasons_played, crowns, finals, best_popularity, secrets_cracked, times_unmasked, created_at' as const;

/** Palmares d'un agent, a URL stable: c'est la fiche que l'on partage. */
export async function fetchAgentCareer(configId: string): Promise<AgentCareer | null> {
  const demo = getDemoCareer(configId);
  if (demo) return demo;
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) throw error;
  return (data as AgentCareer | null) ?? null;
}

/** Classement general, toutes saisons confondues. */
export async function fetchLeaderboard(limit = 20): Promise<AgentCareer[]> {
  const { data, error } = await supabase
    .from('agent_career')
    .select(CAREER_COLUMNS)
    .gt('seasons_played', 0)
  ...
  // Sans base joignable, le classement de demonstration garde la page lisible.
  if (error || !data?.length) return getDemoLeaderboard(limit);
  return data as AgentCareer[];
}

/**
 * Ce qui s'est passe pour les agents du visiteur depuis `since`.
 *
 * C'est la boucle de retour du jeu: on revient pour savoir ce que son IA a
 * fait sans nous, et relire cinquante evenements pour le comprendre n'arrive
 * jamais.
 */
export async function fetchOwnerDigest(
  seasonId: string,
  since?: string
): Promise<OwnerDigest | null> {
  if (isDemoSeason(seasonId)) return null;
  const { data, error } = await supabase.rpc('owner_digest', {
    p_season_id: seasonId,
    p_since: since ?? null,
  });
  if (error) return null;
  return data as OwnerDigest;
}
...
const GUESS_ERRORS: Record<string, string> = {
  already_guessed_today: 'Vous avez deja tente votre chance sur cet agent aujourd hui.',
  own_agent: 'Vous connaissez deja le secret de votre propre agent.',
  agent_eliminated: 'Cet agent est deja hors jeu.',
  season_not_live: "La saison n'est pas en cours.",
  empty_guess: 'Proposez un mot.',
  not_authenticated: 'Connectez-vous pour participer.',
};

/**
 * Propose un mot secret. Le serveur ne renvoie jamais le secret, seulement le
 * verdict: une reponse detaillee permettrait de balayer un dictionnaire.
 */
export async function submitGuess(agentId: string, guess: string): Promise<GuessResult> {
  const { data, error } = await supabase.rpc('submit_spectator_guess', {
    p_agent_id: agentId,
    p_guess: guess,
  });
  if (error) throw error;
  ...
  if (!res.ok) {
    throw new Error(GUESS_ERRORS[res.error ?? ''] ?? res.error ?? 'Proposition refusee');
  }
  return res;
}

export type MyGuesses = {
  ok: boolean;
  day?: number;
  points?: number;
  guessed_today?: string[];
  cracked?: string[];
};

export async function fetchMyGuesses(seasonId: string): Promise<MyGuesses | null> {
  if (isDemoSeason(seasonId)) return null;
  const { data, error } = await supabase.rpc('my_guesses', { p_season_id: seasonId });
  if (error) return null;
  return data as MyGuesses;
}
...
export async function fetchSleuths(limit = 10): Promise<Sleuth[]> {
  const { data, error } = await supabase
    .from('sleuth_leaderboard')
    .select('user_id, display_name, points, correct_guesses, first_bloods, attempts')
    .order('points', { ascending: false })
    .limit(limit);
  if (error) return [];
  return (data ?? []) as Sleuth[];
}
...
export type Challenge = {
  ...
  max_agents?: number;
  enrolled?: number;
  challenger?: { name: string; avatar_url: string; config_id: string; rating: number };
};
...
/** Lisible sans compte: c'est le point d'entree du lien partage. */
export async function viewChallenge(token: string): Promise<Challenge> {
  const { data, error } = await supabase.rpc('view_challenge', { p_token: token });
  if (error) throw error;
  return data as Challenge;
}

export async function acceptChallenge(token: string, configId: string): Promise<Challenge> {
...
/**
 * Preleve le droit d'entree sur le solde et alimente la cagnotte de la saison.
 *
 * Remplace `createEntryPayment`, qui inserait un paiement « pending » que rien
 * ne confirmait jamais: l'inscription etait donc gratuite en pratique.
 */
export async function payEntryFee(
  seasonId: string,
  configId: string
): Promise<{ ok: boolean; error?: string; required?: number; balance?: number }> {
...
/**
 * Annule une saison et rembourse les droits d'entree.
 *
 * Reservee aux admins, et refusee sur une saison terminee: on ne reecrit pas
 * un palmares apres coup.
 */
export async function cancelSeason(
  seasonId: string
): Promise<{ ok: boolean; error?: string; title?: string; refunds?: number; refunded_total?: number }> {
  const { data, error } = await supabase.rpc('cancel_season', { p_season_id: seasonId });
  if (error) throw error;
  return data as { ok: boolean };
}
