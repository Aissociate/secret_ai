...
import { Calendar, Users, Trophy, Plus, Check, Clock, Zap, ArrowRight, DollarSign, Percent, MessageSquare, Gift, Pause, Trash2 } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { useAuth } from '../context/AuthContext';
import { payEntryFee, cancelSeason } from '../api/client';
...
export function SeasonDraftPage() {
  ...
  async function handleEnroll(seasonId: string) {
    ...
    /*
      Le droit d'entree est preleve AVANT l'inscription: l'inverse laissait
      entrer un agent dont le paiement echouait ensuite en silence, et le
      try/catch precedent avalait justement cette erreur.
    */
    if (season && season.entry_fee_usdc > 0) {
      try {
        const paid = await payEntryFee(seasonId, selectedConfig);
        if (!paid.ok) {
          setEnrollError(
            paid.error === 'insufficient_balance'
              ? `Solde insuffisant : ${paid.required} USDC requis, vous avez ${Number(paid.balance ?? 0).toFixed(2)}.`
              : "Le droit d'entree n'a pas pu etre preleve."
          );
          setEnrolling(null);
          return;
        }
      } catch (e) {
        setEnrollError(errorMessage(e, "Le droit d'entree n'a pas pu etre preleve."));
        setEnrolling(null);
        return;
      }
    }
  ...
  /*
    Annulation d'une saison. La confirmation est explicite et nomme la saison:
    l'operation est irreversible et emporte agents, evenements et indices.
  */
  async function handleCancel(seasonId: string, title: string) {
    const ok = window.confirm(
      [
        `Annuler « ${title} » ?`,
        '',
        'La saison, ses agents, ses evenements et ses indices seront supprimes. ' +
          "Les droits d'entree deja payes seront rembourses.",
        '',
    ...
    setCancelling(seasonId);
    try {
      const res = await cancelSeason(seasonId);
      if (!res.ok) {
        setEnrollError(
          res.error === 'season_ended'
            ? "Une saison terminee ne s'annule pas."
            : res.error === 'admin_required'
            ? 'Action reservee aux administrateurs.'
            : (res.error ?? "L'annulation a echoue.")
        );
        return;
      }
      setEnrollError(null);
      ...
      setEnrollError(errorMessage(e, "L'annulation a echoue."));
    } finally {
      setCancelling(null);
    }
  }
  ...
  return (
    <div className="space-y-8">
        ...
        {showCreateForm && effectiveRole === 'admin' && (
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            {effectiveRole === 'admin' && (
                              <button
                                ...
                                className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold text-red-300 bg-red-500/10 border border-red-400/25 hover:bg-red-500/20 transition-colors disabled:opacity-40 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400"
                              >
                                <Trash2 className="w-3 h-3" />
                                {cancelling === s.id ? 'Annulation...' : 'Annuler'}
                              </button>
                            )}
                          </div>
                        </div>
                          ...
                          <div className="space-y-3">
                            ...
                            {canEnrollMore(s) && !isFull && (
                              <div className="flex items-center gap-3 flex-wrap">
                                ...
                                {enrollError && (
                                  <p role="alert" className="w-full text-xs text-amber-300 mt-1">
                                    {enrollError}{' '}
                                    <Link to="/settings/account" className="underline hover:text-amber-200">
                                      Alimenter mon solde
                                    </Link>
                                  </p>
                                )}
            ...
            {liveSeasons.length > 0 && (
              <section className="space-y-4">
                ...
                <div className="grid gap-4">
                  {liveSeasons.map((s) => (
                    /*
                      La carte n'est plus un lien: le bouton d'annulation y
                      serait imbrique, ce qui est du HTML invalide et casse la
                      navigation clavier. Le lien couvre desormais le titre.
                    */
                    <div
                      key={s.id}
                    ...
                    >
                      <div>
                        <Link
                          to={`/show/${s.id}/live`}
                          className="text-lg font-bold hover:text-teal-300 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-400 rounded"
                        >
                          {s.title}
                        </Link>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Link
                          ...
                          className="focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-400 rounded"
                        >
