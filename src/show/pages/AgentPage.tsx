...
function EventRow({
  ...
  /*
    Conteneur non interactif: les pastilles d'agent sont des liens, et un <a>
    imbrique dans un <button> est du HTML invalide — React le signale et la
    navigation clavier est cassee (Entree declenche toujours le conteneur).
  */
  return (
    <div
      role="button"
      ...
      className="w-full text-left border border-white/[0.06] bg-white/[0.01] hover:bg-white/[0.04] hover:border-white/[0.1] rounded-xl p-3.5 cursor-pointer transition-all group/row focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-400"
    >
...
export function AgentPage() {
  ...
  const nextThreshold = useMemo(() => {
    if (!agent) return '';
    const next = nextHintGap(agent.popularity);
    return next
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {agent.agent_config_id && (
                        <Link
                          to={`/agents/${agent.agent_config_id}`}
                          className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-bold text-amber-300 bg-amber-500/10 border border-amber-400/25 hover:bg-amber-500/20 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400"
                        >
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {!isOwnerOfAgent && profile && effectiveRole === 'spectator' && season && (
            <SectionCard className="border-orange-400/10 bg-gradient-to-br from-orange-500/[0.04] to-transparent">
              <div className="flex items-center gap-2 mb-2">
